...
## D43 — El ZIP es la acción por defecto; la carpeta queda al lado

**Contexto.** El spec dice que escribir directo en la carpeta es la mejor experiencia donde existe, y así se implementó primero. Probándolo con cinco fotos el problema saltó solo: guardar exigía contestar un diálogo del sistema para algo que el usuario esperaba resolver de un clic. Aclaración que hacía falta: una descarga común **no deja elegir dónde**, va a la carpeta de descargas sin preguntar. La diferencia real no es «elegir carpeta contra indicar dónde», es un diálogo contra ninguno, y archivos sueltos contra un ZIP que hay que descomprimir.

**Consecuencia.** El botón principal descarga —«Descargar 5 en un ZIP»— y «Guardar en una carpeta» queda al lado, secundario y solo donde el navegador lo soporta. Los dos dicen exactamente lo que hacen, sin que la app adivine por el usuario. Un solo archivo se entrega como archivo suelto y no dentro de un ZIP, y el botón lo dice: «Descargar 1 imagen». El costo aceptado a conciencia: el ZIP se arma entero como `Blob` antes de entregarse, así que en un lote grande hay una segunda copia de todo lo comprimido —respaldada en disco por el navegador, pero copia al fin—, mientras que la carpeta escribe archivo por archivo sin acumular. Por eso la carpeta sigue estando, y por eso es la que conviene en lotes grandes.

## D44 — La promesa de «cero peticiones» se verifica desde fuera de la página, en CI
...
## D47 — La cortina se agarra, no se apunta

**Contexto.** Lo reportó el usuario: un click en cualquier parte de la comparación teletransportaba la línea hasta ahí. Se lee como un fallo y no como un control — nadie espera que mirar de cerca una zona de la foto mueva el divisor. Al arreglarlo aparecieron dos cosas más del mismo origen: el arrastre calculaba el porcentaje contra el área de scroll mientras la línea se posicionaba contra la caja de la imagen, así que en vista 1:1 —donde la caja es más ancha que el área— el arrastre iba descalibrado; y `touch-none` sobre toda la superficie le robaba el scroll a esa misma vista en pantalla táctil.

**Consecuencia.** La cortina es un `role="slider"` con manija visible, foco propio y captura de puntero: se arrastra desde ella y el arrastre sigue aunque el puntero salga. Las flechas la mueven de a 1 %, con Shift de a 10, y Inicio/Fin la llevan a los extremos — sin eso la comparación era solo para ratón, que el piso de calidad del diseño descarta. El porcentaje se mide contra la caja de la imagen, la misma contra la que se posicionan el recorte y la línea. `touch-none` quedó solo en la manija, así que la vista 1:1 vuelve a desplazarse con el dedo. Los oyentes de `pointermove` sobre `window` desaparecieron: la captura de puntero hace el mismo trabajo sin repartir escuchas por todo el documento.

## D48 — Los perfiles de trámite salen del producto

**Contexto.** El eje 2 del spec llamaba a los perfiles por trámite «el diferenciador central», y la v1 salió con la lista vacía a propósito: la estructura lista, ningún perfil, porque un requisito sin fuente verificable no entra. Terminado el despliegue, al pedir el primer trámite concreto para buscarle la fuente oficial, la respuesta fue que no hacía falta ninguno: lo que se necesita es comprimir imágenes. Es el dato más fuerte disponible sobre la demanda real, y viene del único usuario del producto.

**Consecuencia.** Sale el grupo entero: `tramites.ts`, el tipo `TramiteProfile`, `isTramite`, `provenanceOf` y la sección vacía del selector. `Profile` vuelve a ser una sola forma. Los grupos del selector se derivan del catálogo en lugar de estar escritos a mano: la lista escrita a mano existía solo para poder mostrar un grupo sin miembros, y sin ese caso solo podía desviarse de los datos que describe. La regla que el tipo hacía cumplir no se pierde, queda escrita en `types.ts`: si el grupo vuelve, vuelve con `source` y `verifiedAt` obligatorios, porque un límite que nadie puede comprobar es peor que no tener el perfil. Del eje 2 el producto conserva lo que siempre importó: se elige un destino, no un número de calidad.

## D49 — Conservar el formato es la postura por defecto, y el tope de peso se va con los trámites

**Contexto.** Tres de los cuatro perfiles cambiaban la extensión: «Adjunto de correo» y «Enviar por mensajería» forzaban JPEG, «Miniatura» forzaba WebP. Un PNG entraba y salía `.jpg`, sin transparencia y con otro nombre del que el usuario iba a buscar. Conservar el formato choca con el tope de peso: PNG no tiene perilla de calidad —`encodePng` ignora el valor— así que con `format: 'keep'` y `maxBytes` la búsqueda no tiene nada que bisecar y al pipeline solo le queda achicar dimensiones (D21). Cambiar la extensión se ve; perder resolución en silencio, no.
